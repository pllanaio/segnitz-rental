...
## Erster echter CI-Lauf: Auth-Fixture-Isolation

Der erste echte MySQL-CI-Lauf `34754911582` erreichte die fachlichen Tests. Vier Authfälle scheiterten an gemeinsam verbrauchten IP-Limits: Kontoaktivierung/Passwortwechsel/Parallelreset erhielten 429, die beiden konkurrierenden Mutationen erreichten deshalb den Benutzerlock nicht. Die Integrationsfixture verwendet nun ausschließlich für ihren lokalen Server eine ausdrücklich vertraute Loopback-Proxyadresse und je logischem `SessionClient` eine stabile eigene TEST-NET-Adresse, auch für CSRF. Produktionskonfiguration und das tatsächliche Limit von fünf Kontoanfragen bleiben unverändert.

Der tatsächliche SessionClient und der tatsächliche Anwendungs-Limiter wurden in einem Express-HTTP-Verhaltenstest ausgeführt: vor Fix erhielt auch ein neuer Client 429 (rot); danach darf Client A fünf Versuche ausführen, der sechste liefert 429, Client B hat sein eigenes Budget. Von einem nicht vertrauten Peer trennt ein behaupteter Forwarded-Header die Budgets nicht. `node --test test/auth-fixture-isolation.test.js test/proxy-policy.test.js`: **3/3 grün**. Derselbe Fünf-/Sechsversuchsfall ist zusätzlich gegen die echte Passwort-Reset-API vorbereitet. Der nächste GitHub-MySQL-Lauf muss die zuvor blockierten Authassertionen bestätigen; der erste rote CI-Lauf wird nicht als Pass ausgegeben.

Zwei Guest-Lifecycle-Fixtures referenzierten außerdem `mysql`, ohne den Treiber zu importieren; der fehlende Import wurde ergänzt. Keine Anwendungs- oder Datenbankmigration dafür geändert.
